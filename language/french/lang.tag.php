<?php

/**
 * Tag - Language
 *
 * @package		Solspace:Tag
 * @author		Solspace, Inc.
 * @translator		Emmanuel Beauchamps (Put.itin.red studio, Nantes, France)
 * @copyright	Copyright (c) 2008-2016, Solspace, Inc.
 * @link		https://solspace.com/docs/tag
 * @license		https://solspace.com/software/license-agreement
 * @version		4.2.9
 * @filesource	tag/language/french/lang.tag.php
 */

$lang = $L = array(

//----------------------------------------
//	Required for MODULES page
//----------------------------------------

"tag_module_name" =>
"Tag / Étiquette",

"tag_module_description" =>
"Un outil relationnel intuitif",

"tag_module_version" =>
"Tag / Étiquette",


//----------------------------------------
//	Language for Content Wrapper
//----------------------------------------

"tag" =>
"Étiquette",

"online_documentation"	 =>
"Documentation en ligne",

//----------------------------------------
//	Language for home
//----------------------------------------

"homepage" =>
"Page d'accueil",

'utilities' =>
"Utilitaires",

//----------------------------------------
//	Language for urls
//----------------------------------------

"js_urls" =>
"URLs Javascript",

"js_details" =>
"Utiliser ce fragment de code pour incorporer d'utiles fonctions Javascript du module Étiquette à vos formulaires de saisie autonomes (SAEF).",

//----------------------------------------
//	Language for Statistics
//----------------------------------------

"statistics" =>
"Statistiques",

"tags" =>
"Étiquettes",

"top_tags" =>
"Top étiquettes",

"add_tags" =>
"Ajouter des étiquettes",

"total_channel_entries_tagged" =>
"Nombre total d'articles de canal étiquetés",

"percent_channel_entries_tagged" =>
"Pourcentage des articles de canal étiquetés",

"top_five_tags" =>
"Cinq premières étiquettes",

"search_tags" =>
"Rechercher des étiquettes...",

"all_tags" =>
"Toutes les étiquettes",

"harvest_description" =>
"Cet outil récoltera automatiquement pour vous les étiquettes à partir d'un champ de canal initialement utilisé pour l'étiquetage ou pour stocker des mots clefs. Ceci permettra de créer des étiquettes à partir de chaque mot et de les séparer par des espaces. Il n'est pas recommandé que vous utilisiez cette fonctionnalité sur des champs qui contiennent des phrases entières (comme les champs Corps, Résumé ou Titre) car vous créeriez alors une étiquette pour chaque mot !",

'harvest_sources_desc' =>
"Pour récolter les champs étiquettes dans les canaux, vous devez d'abord les définir par canaux dans les préférences.",

//----------------------------------------
//	Language for tag list
//----------------------------------------

'tag_id'						=>
"ID d'étiquette",

"no_tags" =>
"Il n'existe actuellement aucune étiquette.",

"tag" =>
"Étiquette",

"count" =>
"Compte",

"tag_count" =>
"Compte",

"author" =>
"Auteur",

"screen_name" =>
"Pseudo",

"entered_date" =>
"Date saisie",

"entry_date" =>
"Date de l'article",

"edit_date" =>
"Dernière date d'édition",

'tag_added_by' =>
"Ajouté par",

"delete" =>
"Supprimer",

"tag_type" =>
"Type",


//----------------------------------------
//	Language for manage tags
//----------------------------------------

"manage_tags" =>
"Gérer les étiquettes",

//----------------------------------------
//	Language for entries by tag
//----------------------------------------

'entry_title' =>
"Titre de l'article",

'entry_id' =>
"ID de l'article",


"edit_entries_for_" =>
"Éditer les articles pour ",

"channel_entries_by_tag" =>
"Articles de canal par étiquette",

"channel_entries_by_tag_name" =>
"Articles de canal pour \"%tag_name%\"",

"view_channel_entries_for_tag" =>
"Voir les articles de canal pour cette étiquette.",

"title" =>
"Titre",

"no_entries_found" =>
"Il n'y a actuellement aucun article pour cette étiquette.",

//----------------------------------------
//	Language for browse tags
//----------------------------------------

"browse_tags" =>
"Parcourir les étiquettes",

'browse_tags_by' =>
"Parcourir les étiquettes par",

"browse_instructions" =>
"Saisissez dans le champ ci-dessous afin de parcourir les étiquettes.",

'no_tags_found' =>
"Aucune étiquette trouvée",

'no_bad_tags_found' =>
"Aucune mauvaise étiquette trouvée",

//----------------------------------------
//	Language for edit tags
//----------------------------------------

"edit_tag" =>
"Éditer les étiquettes",

"update_tag" =>
"Mettre à jour les étiquettes",

'tag_entries' =>
"Articles",

'tag_view_entries' =>
"Voir les articles",

"tag_exists" =>
"L'étiquette %tag_name% a déjà été ajoutée.",

"tags_combined" =>
"Tous les articles possédant l'étiquette '%old_tag_name%' ont été ré-étiquetés '%new_tag_name%' avec succès.",

"tag_updated" =>
"Votre étiquette a été mise à jour.",

//----------------------------------------
//	Language for manage bad tags
//----------------------------------------

"bad_tag" =>
"Mauvaise étiquette",

"bad_tags" =>
"Mauvaises étiquettes",

"tag_mark_as_bad_" =>
"Marquer l'étiquette comme étant mauvaise ",

"bad_tag_explanation_long" =>
"Les mauvaises étiquettes correspondent à des mots, mis en liste noir, que vous ne souhaitez pas voir soumis au système.",

"manage_bad_tags" =>
"Gérer les mauvaises étiquettes",

"no_bad_tags" =>
"Il n'y a actuellement aucune mauvaise étiquette.",

//----------------------------------------
//	Language for edit bad tags
//----------------------------------------

"add_bad_tag" =>
"Inclure comme mauvaise étiquette",

"add_bad_tags" =>
"Ajouter de mauvaises étiquettes",

"add_bad_tags_instructions" =>
"Séparer chaque mauvaise étiquette par un retour à la ligne.",

"tag_name" =>
"Étiquette",

"tag_name_required" =>
"Un nom d'étiquette est requis.",

"bad_tag_exists" =>
"La mauvaise étiquette %tag_name% a déjà été ajoutée.",

"bad_tag_added" =>
"Les mauvaises étiquettes suivantes ont déjà été ajoutées : %tag_name%.",

"bad_tags_added" =>
"Les mauvaises étiquettes suivantes ont été ajoutées : %tag_name%.",

//----------------------------------------
//	Language for delete tags
//----------------------------------------

"tag_delete_confirm" =>
"Confirmation de suppression d'étiquette",

"tag_delete_question" =>
"Êtes-vous certain de vouloir supprimer %i% %tags% ?",

"action_can_not_be_undone" =>
"Cette action ne peut pas être annulée.",

"tag_deleted" =>
"%i% étiquette supprimée avec succès.",

"tags_deleted" =>
"%i% étiquettes supprimées avec succès.",

//----------------------------------------
//	Language for delete bad tags
//----------------------------------------

'tag_remove' =>
"Sortir",

'remove_bad_tags' =>
"Sortir des mauvaises étiquettes",

"bad_tag_delete_confirm" =>
"Confirmation de sortie de la liste des mauvaises étiquettes",

"bad_tag_delete_question" =>
"Êtes-vous certain de vouloir sortir %i% étiquette(s) (%tags%) de la liste des mauvaises étiquettes ?",

"action_can_not_be_undone" =>
"Cette action ne peut pas être annulée.",

"bad_tag_deleted" =>
"%i% étiquette sortie de la liste des mauvaises étiquettes avec succès.",

"bad_tags_deleted" =>
"%i% étiquettes sorties de la liste des mauvaises étiquettes avec succès.",

// -------------------------------------
//	tag field sync
// -------------------------------------

'sync_tags_fields' =>
"Synchroniser les champs d'étiquette",

'update_tag_field' =>
"Mettre à jour le champ d'étiquette",

'update_tag_field_purpose' =>
"Ceci mettra à jour via AJAX les étiquettes de chaque article et vous préviendra une fois tous les articles traités. Cliquer sur 'Synchoniser les champs d'étiquette' pour démarrer.",

'update_all_tag_counts' =>
"Mettre à jour les compteurs pour Toutes les étiquettes",

"updating_tag_fields_for_entry_" =>
"En cours de mise à jour des champs d'étiquette pour l'article : ",

'number_of_entries_updated' =>
"Nombre d'articles mis à jour",

'tag_fields_complete' =>
"Tous les champs d'étiquette ont été traités.",

'no_tag_custom_field_type' =>
"Vous n'avez aucun champ personnalisé d'étiquette à synchroniser.",

'no_tag_fields_needed_updating' =>
"Aucun champ d'étiquette n'avait besoin d'une mise à jour.",

'tag_field_updated' =>
"Champ d'étiquette mis à jour",

'press_enter_after_each_tags' =>
"Faîtes [Entrer] après chaque étiquette",

// -------------------------------------
//	tag counts
// -------------------------------------

'update_tag_counts' =>
"Mettre à jour les compteurs d'étiquette",

'update_tag_count_from_upgrade_notice' =>
"La mise à jour 4.1.0 du module introduit le principe des compteurs de groupe d'étiquette et doit donc mettre à jour le compteur du nombre total d'article pour chacune des étiquettes.",

'update_tag_count_purpose' =>
"Ceci mettra à jour les compteurs d'étiquette un par un via AJAX et vous préviendra une fois que toutes les étiquettes seront recomptées. Cliquer sur 'Mettre à jour tous les compteurs d'étiquette' pour démarrer.",

'update_all_tag_counts' =>
"Mettre à jour tous les compteurs d'étiquette",

"updating_counts_for_tag_" =>
"Mise à jour du compteur pour l'étiquette : ",

'percent_completed' =>
"Pourcentage d'achèvement",

'number_of_tags_updated' =>
"Nombre d'étiquettes mises à jour",

'wrong_value' =>
"Vous avez communiqué un type de valeur incorrecte",

'pause' =>
"Pause",

'resume' =>
"Reprendre",

'return_to_tag_mcp' =>
"Vous pouvez désormais revenir au Panneau de Contrôle de Tag / Étiquette.",

'tag_recounts_complete' =>
"Tous les recomptages d'étiquette sont achevés.",

//----------------------------------------
//	Language for manage prefs
//----------------------------------------

"tag_preferences" =>
"Préférences",

'site_switcher' =>
"Sélecteur de site",

'channel_switcher' =>
"Sélecteur de canal",

"site_preferences" =>
"Préférences de site",

"site_preferences_for_" =>
"Préférences de site pour ",

'channel_preferences' =>
"Préférences de canal",

'channel_preferences_for_' =>
"Préférences de canal pour ",

'no_channels_for_site' =>
"Aucun canal pour le site",

'tag_preferences_updated' =>
"Préférences mises à jour",

"tag_module_separator" =>
"Séparateur d'étiquette",

'tag_module_separator_subtext' =>
"Ceci est utilisé en frontal lors de l'analyse syntaxique des termes de recherche des étiquettes ou des saisies d'étiquettes personnalisées pour les formulaires de saisie autonomes (SAEF).",

"tag_module_convert_case" =>
"Convertir toutes les étiquettes en minuscules ?",

"tag_module_allow_tag_creation_publish" =>
"Autoriser la création d'une nouvelle étiquette via l'onglet CONTENU : Publier ?",

"tag_module_allow_tag_creation_publish_subtext" =>
"Si le choix est à 'Non', aucune NOUVELLE étiquette ne pourra être créée : ni dans l'espace de
publication, ni via l'onglet CONTENU : Publier, ni dans un champ de canal personnalisé 'étiquette'.
Seules les étiquettes existantes pourront être soumises pour un article. Ceci permet à un Administrateur de contrôler les étiquettes qui peuvent être affectées aux articles.",

"tag_module_publish_entry_tag_limit" =>
"Nombre maximum d'étiquettes autorisées par article dans l'onglet CONTENU : Publier",

'tag_preference_maximum_tags_allowed' =>
"%n% étiquette(s) autorisée(s) au maximum",

"tag_module_enable_tag_form" =>
"Activer le formulaire d'étiquette pour les visiteurs du site ?",

"y" =>
"Oui",

"n" =>
"Non",

"tag_field" =>
"Étiquettes",

"tag_harvest_field" =>
"Champ personnalisé de récolte d'étiquette",

"tag_harvest_field_for_" =>
"Champ personnalisé de récolte d'étiquette pour",

"tag_harvest_field_subtext" =>
"Si, à tout moment, vous décidez d'utiliser l'outil de récolte d'étiquette : sachez que toutes les valeurs de type Texte présentes dans ce champ personnalisé seront converties en Étiquettes lors de leur soumission à l'outil de récolte. Cette fonction prend en compte la préférence de séparateur d'étiquette.",

"preferences_for_site" =>
"Préférences pour le site",

"preferences_for_" =>
"Préférences pour",

"publish_tab_label" =>
"Label de l'onglet Publication",

"publish_tab_label_for_" =>
"Label de l'onglet Publication pour",

"publish_tab_label_subtext" =>
"Laissez ce champ à blanc afin de désactiver l'onglet de Publication du module Tag / Étiquette pour un canal donné.",

"publish_tab_label_deprecated" =>
"Saviez-vous qu'Étiquette est désormais un type de champ ? C'est la façon recommandée de l'utiliser dans l'espace publication. Ainsi l'utilisation de ce label de l'onglet de Publication est désapprouvée, mais elle est conservée à titre historique.",

'choose_custom_field' =>
"Choisir un champ personnalisé",

'explode_input_on_separator' =>
"Séparer des étiquettes multiples avec un délimiteur",

'explode_input_on_separator_subtext' =>
"Cette option vous permet de saisir en une seule fois des étiquettes multiples dans le champ étiquette, en les séparant par le délimiteur choisi. Par exemple, cette option étant activée et le délimiteur positionné à 'virgule', le fait de saisir 'un, deux, trois' rajoutera 3 étiquettes dans votre champ étiquette : 'un', 'deux' et 'trois'.",

"explode_input_on_separator_note" =>
"Le texte séparé par un(e) <span class=\"sep\">%sep%</span> sera séparé en étiquettes multiples.",

"select_delimiter" =>
"Sélectionner le délimiteur : ",

"enable_explode_controls" =>
"Activer les contrôles utilisateurs pour séparer des étiquettes multiples avec un délimiteur",

"enable_explode_controls_subtext" =>
"Permet aux utilisateurs d'avoir des contrôles supplémentaires, tel que celui de choisir le délimiteur de texte, lors de l'activation de la préférence ci-dessus.",

//--------------------------------------------
//	tag groups
//--------------------------------------------

'viewing_tags_for_group' =>
"Visualisation des étiquettes pour le groupe ",

'view_tags_in_all_groups' =>
"Voir les étiquettes dans tous les groupes",

'undefined_tag_group' =>
"Groupe d'étiquette non défini",

'tag_groups' =>
"Groupes d'étiquette",

'tag_group' =>
"Groupe d'étiquette",

'group_id' =>
"ID de groupe",

'group_name' =>
"Nom du groupe",

'group_short_name' =>
"Nom court",

'delete_tag_groups' =>
"Supprimer Groupe(s)",

'insert_new_tag_group' =>
"Insérer un nouveau groupe d'étiquette",

'cancel' =>
"Annuler",

'error' =>
"Erreur",

'tag_limit_reached' =>
"Limite du nombre d'étiquette atteinte : %num%",

'new_group_name' =>
"Nom du nouveau groupe",

'short_name' =>
"Nom court",

'tag_group_subtext' =>
"Les groupes d'étiquette permettent d'aller au delà de la limite d'un seul champ d'étiquette par canal. Les noms des nouveaux groupes d'étiquette ne peuvent comporter que des lettres, des chiffres et des soulignés (underscore).",

"tag_group_name_taken" =>
"Le nom de groupe d'étiquette que vous venez de saisir possède le même nom court qu'un autre groupe déjà existant. Merci de choisir un autre nom pour ce groupe d'étiquette.",

'all_open' =>
"Tout ouvert",

'all_open_subtext' =>
"Définit si les champs 'Suggérer des étiquettes' et 'Top étiquettes' de l'espace de publication sont tous les deux ouverts par défaut ou non.",

'delete_tag_groups' =>
"Supprimer Groupe(s)",

'tag_groups' =>
"Groupes d'étiquette",

'tag_group_id' =>
"ID de groupe",

'tag_group_name' =>
"Nom du groupe",

'tag_group_short_name' =>
"Nom court",

'locked' =>
"Verrouillé",

'total_tags_in_group' =>
"Nombre total d'étiquettes dans le groupe",

'view_group_tags' =>
"Voir les étiquettes du groupe",

'view_tags_in_group' =>
"Voir les étiquettes dans le groupe",

"tag_group_delete_confirm" =>
"Confirmation de suppression du groupe d'étiquette",

"tag_group_delete_question" =>
"Êtes-vous certain de vouloir supprimer %i% groupe(s) d'étiquette ?",

"tag_group_deleted" =>
"Groupe(s) d'étiquette supprimé(s) avec succès.",

'create_tag_group' =>
"Créer Groupe",

'update_tag_group' =>
"Mettre à jour Groupe",

'edit_tag_group' =>
"Éditer le groupe d'étiquette",

'tag_group_updated' =>
"Groupe d'étiquette mis à jour",

'tag_group_created' =>
"Groupe d'étiquette créé",

'locked_tag_group_description' =>
"Les groupes d'étiquette actuellement utilisés dans les champs personnalisés sont verrouillés jusqu'à ce que ce champ soit sorti de son groupe.",

'suggest_from' =>
"Autocompléter, Suggérer des étiquettes et Top étiquettes venant de",

'suggest_from_subtext' =>
"Normalement, les champs personnalisés d'étiquette n'afficheront que des résultats autocomplétés, tout comme ils ne vont suggérer des étiquettes et montrer des top étiquettes ne provenant que de leur propre groupe de référence. Vous pouvez toutefois utiliser ce paramètre pour afficher des résultats autocomplétés, suggérer des étiquettes et montrer des top étiquettes provenant de TOUS les groupes d'étiquette.",

'this_group' =>
"Ce groupe",

'all_groups' =>
"Tous les groupes",

'top_tag_limit' =>
"Limite du nombre de Top étiquettes",

'top_tag_limit_subtext' =>
"Limite le nombre de Top étiquettes affichées.",

//----------------------------------------
//	Language for harvest
//----------------------------------------

'tag_harvest' =>
"Récolter",

'harvest_tags' =>
"Récolter des étiquettes à partir de champs pré-existants",

"tag_harvest_instructions" =>
"Vous pouvez récolter des données dans votre site, soit en créant une correspondance
entre les catégories et les étiquettes, soit en transformant le contenu d'un champ personnalisé
en étiquettes. Dans les deux cas, les articles seront affectés aux étiquettes récoltées
de manière appropriée.",

"categories" =>
"Catégories",

"galleries" =>
"Galeries",

'harvest_sources' =>
"Sources de récolte",

'per_batch' =>
"Taille du lot",

"harvest_from_channel_categories" =>
"Catégories de canal",

"harvest_from_channel_tag_field" =>
"Champs d'étiquette pour les canaux",

"tag_harvest_batch_process" =>
"Traitement des lots de récolte",

"tag_process_batch_of" =>
"Traitement de %batch% sur %total%",

"start" =>
"Démarrage",

"continue" =>
"Continuer",

"done" =>
"Terminé",

'error_invalid_custom_fields_for_channels' =>
"Erreur : dans vos préférences d'étiquette, un des champs personnalisé de canal est invalide.",

'success_harvest_processing_is_complete' =>
"Succès ! Le traitement de récolte est achevé !",

//----------------------------------------
//	Language for edit CP
//----------------------------------------

"tag_field_instructions_" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un retour à la ligne.",

"tag_field_instructions_linebreak" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un retour à la ligne.",

"tag_field_instructions_comma" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec une virgule.",

"tag_field_instructions_colon" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un deux-points.",

"tag_field_instructions_semicolon" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un point-virgule.",

"tag_field_instructions_space" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un espace.",

"tag_field_instructions_doublepipe" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec une double barre verticale.",

"tag_field_instructions_pipe" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec une barre verticale (|).",

"tag_field_instructions_newline" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un retour chariot.",

"tag_field_instructions_tilde" =>
"Utilisez ce champ pour saisir à votre guise les étiquettes pour votre article. Séparez chaque étiquette avec un tilde (~).",

"tag_browser" =>
"Navigateur d'étiquette",

"tag_browser_instructions" =>
"Saisissez dans le champ ci-dessous pour rechercher des étiquettes. Cliquez pour ajouter une étiquette à un article. Listez toutes les étiquettes disponibles en mettant un astérisque (*).",

"tag_browser_results" =>
"Résultats de la recherche d'étiquette",

"tag_collector" =>
"Collecteur d'étiquette",

"searching" =>
"Recherche...",

"or" =>
"OU",

"suggest_tags" =>
"Suggérer des étiquettes",

'search_all_sites'				=>
"Rechercher sur tous les sites",

"type_above" =>
"Merci de saisir dans le champ ci-dessus.",

"no_matching_tags" =>
"Impossible de trouver des étiquettes correspondantes.",

'popular_tags' =>
"Étiquettes les plus populaires",

'popular_tags_instructions' =>
"Cliquer pour ajouter l'étiquette à l'article.",


// -------------------------------------
//	separators
// -------------------------------------

'separator_colon'		=>
'Deux-points \':\'',

'separator_comma'		=>
'Virgule \',\'',

'separator_doublepipe'	=>
'Double barre verticale \'||\'',

'separator_newline'		=>
'Retour chariot \'\\\n\'',

'separator_pipe'		=>
'Barre verticale \'|\'',

'separator_semicolon'	=>
'Point-virgule \';\'',

'separator_space'		=>
'Espace \' \'',

'separator_tab'			=>
'Tabulation \'\\\t\'',

'separator_tilde'		=>
'Tilde \'~\'',

//----------------------------------------
//	Language for insert tags
//----------------------------------------

"missing_entry_id" =>
"L'ID de l'article manque.",

"no_tags_submitted" =>
"Aucune étiquette n'a été soumise.",

"not_authorized" =>
"Vous n'êtes pas autorisé à soumettre ce formulaire.",

//----------------------------------------
//	Tag Recount Tab
//----------------------------------------

'recount' =>
"Recalculer les statistiques",

'tag_recount' =>
"Recalculer les statistiques de groupe d'étiquette",

'no_tags_to_recount'			=>
"Il n'y avait aucune étiquette à recompter.",

'tags_successfully_recounted' 	=>
"Les étiquettes ont été recomptées avec succès",

'tag_recount_running'			=>
"Comptage de %num étiquettes à partir de %start. Nombre total d'étiquettes : %total.",

//----------------------------------------
// Backport Tags from Tab
//----------------------------------------

'tag_sync' =>
"Synchroniser",

'tag_field_sync' =>
"Synchro du champ d'étiquette",

'tag_field_sync_subtext' =>
"Synchronisez les étiquettes avec le champ personnalisé d'étiquette afin qu'elles puissent être recherchées avec le module Recherche.
Attention, c'est seulement nécessaire si vous aviez déjà utilisé l'onglet de Publication du module Tag / Étiquette.",

//----------------------------------------
//	Tag Subscription
//----------------------------------------

'successful_tag_subscribe' =>
"Vous avez été abonné à l'étiquette avec succès.",

'successful_tag_unsubscribe' =>
"Vous avez été désabonné de l'étiquette avec succès.",

'tag_subscribed' =>
"Abonné à cette étiquette",

'tag_unsubscribed' =>
"Désabonné de cette étiquette",

//----------------------------------------
//----------------------------------------
//	General
//----------------------------------------

"submit" =>
"Soumettre",

"member" =>
"Membre",

"search" =>
"Rechercher",

"edit" =>
"Éditer",

"action_can_not_be_undone" =>
"Cette action ne peut pas être annulée.",

// --------------------------------------------
//  Errors
// --------------------------------------------

"error_tag_parsing" =>
"Une erreur d'analyse syntaxique d'étiquette s'est produite.",

'tag_module_disabled' =>
"Le module Étiquette est actuellement désactivé. Merci de vous assurer qu'il est installé et à jour en visitant le panneau de contrôle du module dans le Panneau de Contrôle ExpressionEngine.",

'disable_module_to_disable_extension' =>
"Pour désactiver cette extension, vous devez désactiver son <a href='%url%'>module</a> correspondant.",

'enable_module_to_enable_extension' =>
"Pour activer cette extension, vous devez installer son <a href='%url%'>module</a> correspondant.",

'cp_jquery_requred' =>
"L'extension 'jQuery pour le Panneau de Contrôle' doit être <a href='%extensions_url%'>activée</a> afin d'utiliser ce module.",

// --------------------------------------------
//  Update Routine
// --------------------------------------------

'update_tag_module' =>
"Mettre à jour le module Tag / Étiquette",

'tag_update_message' =>
"Vous avez récemment téléchargé une nouvelle version de Tag / Étiquette, merci de cliquer ici pour exécuter le script de mise à jour.",

'update_successful' =>
"Succès de la mise à jour !",

"upgrade_message" =>
"Il semble que vous ayez installé une nouvelle version de Tag / Étiquette. Nous vous recommandons d'exécuter le script de mise à jour.",

"update_successful" =>
"Le module a été mis à jour avec succès.",

"update_failure" =>
"Une erreur s'est produite au moment de la tentative de mise à jour de votre module vers la nouvelle version.",


// END
''=>''
);
